...
print("Debug:", np.isfinite(df["ss_temp"]).all())

# Print the summary statistics:
print("Sumarry statistics:", df.describe())
...
# Inspect humidity missing behaviour in more detail:
# Create 2D array: 1 if missing, 0 if measured
nan_array = df["humidity"].isna().astype(int).to_numpy().reshape(1, -1)

plt.figure(figsize=(14, 3))
plt.imshow(nan_array, aspect="auto", cmap="gray_r", interpolation="nearest")

plt.title("Missing Humidity Over Time (white = missing)")
plt.yticks([])  # no y-axis labels since it's just one row

# Use actual dates on x-axis
n_rows = len(df)
n_ticks = min(10, n_rows)  # pick up to 10 ticks for readability
tick_positions = np.linspace(0, n_rows - 1, n_ticks).astype(int)
tick_labels = df["date"].iloc[tick_positions].dt.strftime("%Y-%m-%d")
...
# Violinplot to show monthly ditribution
df["month_cat"] = pd.Categorical(df["month"], categories=range(1, 13), ordered=True)

plt.figure(figsize=(12, 6))
sns.violinplot(x="month_cat", y="ss_temp", data=df)
plt.title("Distribution of Sea Surface Temperature by Month (all years combined)")
plt.xlabel("Month")
plt.ylabel("Sea Surface Temperature (°C)")
plt.show()

# Boxplot to also show monthly distribution, have to decide which I want to choose!
plt.figure(figsize=(12, 6))
sns.boxplot(x="month_cat", y="ss_temp", data=df, palette="Set3")
plt.title("Boxplot of Sea Surface Temperature by Month (all years combined)")
plt.xlabel("Month")
plt.ylabel("Sea Surface Temperature (°C)")
plt.show()
...
feature_columns = ["zon_winds", "mer_winds", "humidity", "air_temp", "ss_temp"]

# Subset and ensure numeric
df_numeric = (
    df[feature_columns + ["weight"]].apply(pd.to_numeric, errors="coerce").dropna()
)

# Extract weights and normalize
w = df_numeric["weight"].values
w /= w.sum()  # normalize to sum=1

# Extract data matrix (without weight column)
...
weighted_cov = np.cov(X_centered.T, aweights=w, bias=False)
...
fig_weighted = ff.create_annotated_heatmap(
    z=weighted_corr,
    x=feature_columns,
    y=feature_columns,
    colorscale="Viridis",
...
fig_weighted.update_layout(
    title="Weighted Correlation Heatmap (using yearly weights)",
    xaxis_title="Features",
    yaxis_title="Features",
)
